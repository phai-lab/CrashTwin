...
def demo(opt):
  ...
  while True:
      ...
      # raise Exception(f"opt.video_w: {opt.video_w}, opt.video_h: {opt.video_h}")
      if opt.resize_video:
...
def save_and_exit(opt, out=None, results=None, out_name=''):
  if opt.save_results and (results is not None):
    if opt.results_out_path:
      save_dir = opt.results_out_path
    else:
      save_dir = '../results/{}_results.json'.format(opt.exp_id + '_' + out_name)
    save_parent = os.path.dirname(save_dir)
    if save_parent and not os.path.isdir(save_parent):
      os.makedirs(save_parent, exist_ok=True)
    print('saving results to', save_dir)
    with open(save_dir, 'w') as fp:
      json.dump(_to_list(copy.deepcopy(results)), fp)
