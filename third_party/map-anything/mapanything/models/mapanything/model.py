...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # Create a copy of the config before deleting keys to preserve them for serialization
        encoder_config_copy = self.encoder_config.copy()
        # Remove keys that are not accepted by the installed UniCeption encoder implementation
        encoder_config_copy.pop("uses_torch_hub", None)
        encoder_config_copy.pop("norm_returned_features", None)
    ...
    def _initialize_info_sharing(self, info_sharing_config):
        ...
        # Older UniCeption versions expect \"mlp_layer\" to be a callable, not a string.
        # The HF config may provide a string (e.g., \"dummy\" or \"swiglufused\"). In that
        # case, drop it so the default Mlp implementation is used.
        mlp_layer = info_sharing_config["module_args"].get("mlp_layer", None)
